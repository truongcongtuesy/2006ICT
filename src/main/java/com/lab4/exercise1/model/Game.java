...
public class Game {
    ...
    public void updateBallPosition() {
        ...
        // Bounce off edges with proper ball size consideration
        if (nextX - ballSize <= 0 || nextX + ballSize >= fieldWidth) {
            dx = -dx;
            // Keep ball within bounds
            ballX = Math.max(ballSize, Math.min(fieldWidth - ballSize, ballX));
        } else {
            ballX = nextX;
        }
        ...
        if (nextY - ballSize <= 0 || nextY + ballSize >= fieldHeight) {
            dy = -dy;
            // Keep ball within bounds
            ballY = Math.max(ballSize, Math.min(fieldHeight - ballSize, ballY));
        } else {
            ballY = nextY;
        }
    }
     ...
     */
    public void adjustYSpeed(double delta) {
